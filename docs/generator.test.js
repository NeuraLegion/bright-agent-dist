...
// ---------------------------------------------------------------------------
// Workflow behavior HTML
// ---------------------------------------------------------------------------
test("workflowBehavior: no triggers explains the empty selection", () => {
  const h = G.workflowBehavior(st({ triggers: only() }));
  assert.match(h, /Scan behavior by trigger/);
  assert.match(h, /No triggers selected/);
  assert.match(h, /RUN_MODE.*target startup/);
  assert.match(h, /SCAN_SCOPE.*repository coverage/);
  assert.match(h, /RUN_MODE=full.*does not mean a full repository scan/);
  assert.doesNotMatch(h, /<li>/);
});

test("workflowBehavior: every GitHub auto trigger shows its exact event and effective behavior", () => {
  const cases = [
    ["manual", "workflow_dispatch", "Full repository scan"],
    ["schedule", "schedule", "Full repository scan"],
    ["pr", "pull_request", "Changed-files scan"],
    ["push", "push", "Changed-files scan"],
    ["steering", "issue_comment /bright-agent", "Changed-files scan"],
  ];
  for (const [trigger, event, behavior] of cases) {
    const h = G.workflowBehavior(st({ triggers: only(trigger), scope: "auto" }));
    assert.match(h, new RegExp(event.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")), `${trigger} event`);
    assert.match(h, new RegExp(behavior), `${trigger} behavior`);
    assert.equal((h.match(/<li>/g) || []).length, 1, `${trigger} has one row`);
  }
  assert.match(G.workflowBehavior(st({ triggers: only("pr") })), /actual PR base branch.*origin\/&lt;base&gt;/);
  assert.match(G.workflowBehavior(st({ triggers: only("steering") })), /resolved commented PR base.*origin\/&lt;base&gt;/);
  assert.match(G.workflowBehavior(st({ triggers: only("push") })), /github\.event\.before.*previous branch tip/);
});

test("workflowBehavior: mixed manual and PR auto rows keep different coverage", () => {
  const h = G.workflowBehavior(st({ triggers: only("manual", "pr"), scope: "auto" }));
  assert.equal((h.match(/<li>/g) || []).length, 2);
  assert.match(h, /workflow_dispatch<\/code><\/strong>: Full repository scan/);
  assert.match(h, /pull_request<\/code><\/strong>: Changed-files scan/);
});

test("workflowBehavior: explicit full overrides every trigger without diff mechanics", () => {
  const h = G.workflowBehavior(st({
    triggers: only("manual", "schedule", "pr", "push", "steering"),
    scope: "full",
  }));
  assert.equal((h.match(/Full repository scan\./g) || []).length, 5);
  assert.doesNotMatch(h, /<\/strong>: Changed-files scan/);
  assert.doesNotMatch(h, /Diff source:|origin\/&lt;base&gt;/);
  assert.match(h, /pull_request<\/code><\/strong>: Full repository scan\. The trigger filters PRs targeting/);
});

test("workflowBehavior: explicit changed manual gives an actionable base instruction", () => {
  const h = G.workflowBehavior(st({ triggers: only("manual"), scope: "changed" }));
  assert.match(h, /Changed-files scan/);
  assert.match(h, /generated workflow has no event-specific <code>DIFF_BASE<\/code>/);
  assert.match(h, /add <code>DIFF_BASE: \.\.\.<\/code> to the <code>Run Bright Agent<\/code> step's <code>env<\/code> block/);
});

test("workflowBehavior: GitHub CodeQL validation describes generated SARIF without scope claims", () => {
  const h = G.workflowBehavior(st({
    runMode: "validation",
    scope: "changed",
    sarifTool: "codeql",
    triggers: only("manual", "pr", "push"),
  }));
  assert.match(h, /SARIF validation with no fix loop, using CodeQL results generated by this workflow/);
  assert.match(h, /SCAN_SCOPE<\/code> and <code>DIFF_BASE<\/code> are not generated/);
  assert.match(h, /not classified as full or changed/);
  assert.doesNotMatch(h, /<\/strong>: Full repository scan/);
  assert.doesNotMatch(h, /<\/strong>: Changed-files scan/);
});

test("workflowBehavior: external validation distinguishes placeholder and configured SARIF paths", () => {
  const missing = G.workflowBehavior(st({
    platform: "gitlab",
    runMode: "validation",
    sarifTool: "existing",
    sarifPath: "",
    triggers: only("manual"),
  }));
  assert.match(missing, /SARIF_PATH<\/code> is emitted as <code>REPLACE_WITH_PATH_TO\.sarif<\/code>/);
  assert.match(missing, /replace it before this validation can run/);

  const configured = G.workflowBehavior(st({
    platform: "gitlab",
    runMode: "validation",
    sarifTool: "existing",
    sarifPath: `reports/result</code><script>alert("x")</script>.sarif`,
    triggers: only("manual"),
  }));
  assert.match(configured, /Runs SARIF validation from <code>reports\/result&lt;\/code&gt;&lt;script&gt;alert\(&quot;x&quot;\)&lt;\/script&gt;\.sarif<\/code>/);
  assert.doesNotMatch(configured, /<script>/);
  assert.doesNotMatch(configured, /REPLACE_WITH_PATH_TO/);
});

test("workflowBehavior: push always includes lifecycle and scan-branch caveats", () => {
  for (const overrides of [{ scope: "auto" }, { scope: "full" }, { runMode: "validation" }]) {
    const h = G.workflowBehavior(st({ ...overrides, triggers: only("push") }));
    assert.match(h, /Initial branch-creation and branch-deletion pushes are skipped/);
    assert.match(h, /bright-scan-\*/);
    assert.doesNotMatch(h, /two-dot|strict two/i);
  }
});

test("workflowBehavior: Bedrock OIDC preflight appears only with GitHub manual", () => {
  const oidcManual = G.workflowBehavior(bedrockOidc({ triggers: only("manual") }));
  assert.match(oidcManual, /preflight=true<\/code> validates access and model availability without a scan/);
  assert.match(oidcManual, /preflight=false<\/code> is the default and performs the configured scan/);

  assert.doesNotMatch(
    G.workflowBehavior(st({ provider: "bedrock", inferenceAuth: "token", triggers: only("manual") })),
    /preflight=true/,
  );
  assert.doesNotMatch(G.workflowBehavior(bedrockOidc({ triggers: only("pr") })), /preflight=true/);
});

test("workflowBehavior: Bedrock OIDC validation presents preflight as an alternative", () => {
  const h = G.workflowBehavior(bedrockOidc({
    runMode: "validation",
    sarifTool: "codeql",
    triggers: only("manual"),
  }));
  assert.match(h, /preflight=true<\/code> performs only the access\/model check and skips SARIF preparation and validation/);
  assert.match(h, /preflight=false<\/code> is the default and follows the validation path described above/);
  assert.doesNotMatch(h, /performs the configured scan/);
});

test("workflowBehavior: CircleCI explains that schedule replaces the separate manual workflow", () => {
  const s = st({ platform: "circleci", triggers: only("manual", "schedule") });
  const h = G.workflowBehavior(s);
  assert.match(h, /<strong>Manual run<\/strong>: Not emitted separately/);
  assert.match(h, /CircleCI generator emits only the <code>nightly-dast<\/code> workflow/);
  assert.match(h, /<strong>Scheduled run<\/strong>: Full repository scan/);
  assert.match(G.generateCircle(s), /nightly-dast:/);
  assert.doesNotMatch(G.generateCircle(s), /on-demand:/);
});

test("workflowBehavior: escapes the configured default branch", () => {
  const branch = `main</code><script>alert("x")</script>`;
  const h = G.workflowBehavior(st({ defaultBranch: branch, triggers: only("pr") }));
  assert.doesNotMatch(h, /<script>/);
  assert.match(h, /main&lt;\/code&gt;&lt;script&gt;alert\(&quot;x&quot;\)&lt;\/script&gt;/);
});

test("workflowBehavior: non-GitHub rows use human labels and CI context", () => {
  const h = G.workflowBehavior(st({ platform: "gitlab", scope: "changed", triggers: only("pr") }));
  assert.match(h, /<strong>Merge request<\/strong>: Changed-files scan/);
  assert.match(h, /PR\/MR base comes from the CI context/);
  assert.doesNotMatch(h, /github\.event|workflow_dispatch|pull_request|DIFF_BASE/);
});
